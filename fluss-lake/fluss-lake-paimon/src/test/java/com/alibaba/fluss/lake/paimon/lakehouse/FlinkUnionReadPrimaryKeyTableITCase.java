...
public class FlinkUnionReadPrimaryKeyTableITCase extends FlinkUnionReadTestBase {
    ...
    void testPrimaryKeyTable(boolean isPartitioned) throws Exception {
        ...
        long tableId =
                preparePkTable(tablePath, DEFAULT_BUCKET_NUM, isPartitioned, bucketLogEndOffset);
        ...
        // write records again
        writeRowsToPkTable(
                tablePath, tableId, DEFAULT_BUCKET_NUM, isPartitioned, bucketLogEndOffset);
        ...
        if (isPartitioned) {
            expectedRows = paddingPartition(tablePath, expectedRows);
        }
        ...
        if (isPartitioned) {
            ...
            for (String partition : partitionNameById.values()) {
                for (Object[] values : rowsValue) {
                    rows.add(row(appendPartition(values, partition)));
                }
                writeRows(tablePath, rows, false);
            }
        } else {
    ...
    private Object[] appendPartition(Object[] values, String partition) {
