...
//! ClusterRegistry.
//!
//! Knows which clusters exist and their connection config (`default` only,
//! single-cluster). Bottom of the `BackendFacade -> FlussConnectionProvider ->
...
/// Connection configuration for a single cluster. Minimal: just the bootstrap
/// endpoint(s) used to construct the shared proxy connection.
/// Per-user credentials / doAs are out of scope (DESIGN.md §2).
...
/// Minimal cluster registry. Single-cluster: exactly one entry keyed by
/// `ClusterId("default")`. The shape (a map) keeps the lookup call site stable,
/// but no multi-cluster routing table is built (DESIGN.md §2: real multi-cluster
/// is out of scope).
#[derive(Debug, Clone)]
...
impl ClusterRegistry {
    ...
    /// `ClusterNotFound` variant; an unknown cluster is a bad request.)
