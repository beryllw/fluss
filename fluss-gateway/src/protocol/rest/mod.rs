...
impl RestOptions {
    fn from_config(rest: &RestServerConfig, limits: &RequestLimitsConfig) -> Self {
        Self {
            ...
            lookup_max_keys: limits.lookup_max_keys,
            lookup_max_key_bytes: limits.lookup_max_key_bytes.bytes(),
            lookup_max_concurrent_requests: limits.lookup_max_concurrent_requests,
            prefix_lookup_max_prefixes: limits.prefix_lookup_max_prefixes,
            prefix_lookup_max_rows_per_prefix: limits.prefix_lookup_max_rows_per_prefix,
            prefix_lookup_max_concurrent_requests: limits.prefix_lookup_max_concurrent_requests,
        }
    }
}
...
async fn collect_body(request: Request) -> GatewayResult<Bytes> {
...
fn ensure_json_acceptable(headers: &HeaderMap) -> GatewayResult<()> {
...
pub fn build(
    ...
    let state = RestState {
        ...
        lookup_max_keys: options.lookup_max_keys,
        lookup_max_key_bytes: options.lookup_max_key_bytes,
        prefix_lookup_max_prefixes: options.prefix_lookup_max_prefixes,
        prefix_lookup_max_rows_per_prefix: options.prefix_lookup_max_rows_per_prefix,
    };
...
pub fn build_router(state: RestState, options: &RestOptions) -> Router {
    ...
    let (records_router, records_api) = OpenApiRouter::new()
        ...
        .split_for_parts();
    let (lookup_router, lookup_api) = OpenApiRouter::new()
        .merge(lookup::point_routes())
        .split_for_parts();
    let (prefix_lookup_router, prefix_lookup_api) = OpenApiRouter::new()
        .merge(lookup::prefix_routes())
        .split_for_parts();
    ...
    guarded_api.merge(lookup_api);
    guarded_api.merge(prefix_lookup_api);
    ...
    let lookup_router = apply_concurrency_limit(
        lookup_router,
        options.lookup_max_concurrent_requests,
        "too many concurrent primary-key lookup requests",
    )
    .route_layer(middleware::from_fn(reject_query_parameters));
    let prefix_lookup_router = apply_concurrency_limit(
        prefix_lookup_router,
        options.prefix_lookup_max_concurrent_requests,
        "too many concurrent prefix lookup requests",
    )
    .route_layer(middleware::from_fn(reject_query_parameters));
...
pub mod test_support {
    ...
    pub fn test_options() -> RestOptions {
        RestOptions {
            ...
            lookup_max_keys: 128,
            lookup_max_key_bytes: 1024 * 1024,
