...
//! [`SqlEnvironmentProvider`]: the single SQL-environment assembly entry.
//!
...
//! into the caller-supplied `ctx`. Design: `design/sql-path.md`.
//!
//! Responsibility boundary: the provider owns *what is installed
...
//! prepared-statement wire lifecycle all belong to the PostgreSQL adapter,
...
/// Per-protocol SQL environment assembly seam.
...
pub trait SqlEnvironmentProvider: Send + Sync {
    ...
    ///
    /// Only mutations the session layer classified as `ApplyToExistingContext`
    /// (see `SessionMutationEffect`) are meaningful here; everything else is a no-op
