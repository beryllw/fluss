...
> 覆盖：核心接口与请求模型 → `instance/` `types.rs` `error.rs`；Session 与 Operation → `session/`。
> 面向用户的使用说明请先看 [`../README.md`](../README.md)；本文件只保留内部契约与生命周期设计。
...
### GatewayInstance 是唯一的 core facade

Gateway core 对协议模块只暴露一套异步 trait；PostgreSQL、REST 都依赖它，而不各自直接碰 session manager、backend 或 DataFusion。
...
小而稳定的 typed wrapper（`SessionId`、`OperationId`、`ClusterId`、`Principal`、`TableRef { database, table }`）把 transport 层字符串解析与 core 领域对象分开。`ClusterId` 的内部值语义上是 cluster 名称（恒为 `default`）。

### metadata API 走显式 scope
...
metadata 本质 cluster-scoped；REST 本就无 session；PG adapter 从 session snapshot 取 `(principal, cluster)` 再转调即可。

### SQL / direct 结果保持 Arrow-native
...
由各协议 adapter 自己做编码（PG row / JSON / Arrow IPC）。

### SqlExecution::Command 形状

PG 只读，但保留 Command 分支，避免扩展重改核心返回类型：
...
### direct path 请求显式携带 execution context
...
// 权威定义见 direct-path.md；此处与之保持一致。
```

让 direct path 不依赖 SessionManager、cancel/timeout 在 core 层即可表达。

### domain error 只在 core 表达业务语义

- core error 只表达领域语义，不含 PG error code / HTTP status。
...
## Session 与 Operation 核心模型

把 session、`SessionContext`、operation、cancel/timeout 的关系定清楚。

### Session 是连接级对象，direct path 不进 SessionManager

- PostgreSQL：显式 open / close session。
- REST direct path：不创建 session，不进 SessionManager。
...
### OpenSessionRequest 只表达连接建立时确定的信息
...
- PostgreSQL：`sql_environment = Some("postgres")`；direct path 不调 `open_session`。
- `principal`、`cluster`、`sql_environment` 在 session 生命周期内只读；可变项都进 `SessionVars`。
...
- 只有真正跨协议都讲得通的变量进顶层 typed fields；协议局部变量进 `environment`（如 `pg.search_path`、`pg.standard_conforming_strings`、`pg.application_name`）。
- `SessionVars` 是持久状态；`SessionContext` 只是某一代运行时投影。

### Session mutation 必须先改 vars，再处理运行态影响
...
### SessionContext 采用 lazy init + dirty rebuild
...
### Session close 与运行中 operation 解耦
...
### tracked stream 驱动 operation 状态
...
取消/超时触发的错误不记为 `Failed`；`Failed` 只用于真正的执行失败。
...
### CancelRequest 与 client disconnect 语义
...
idle reaper：只回收空闲 session；有 active operations 的不回收；`last_access_at` 至少在 `open_session`、`get_session`、`alter_session`、`describe_sql`、`execute_sql` 时更新。
