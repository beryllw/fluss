...
- `query` keeps rows in structured JSON; MCP text content carries a user-audience SQL echo plus an assistant-audience JSON mirror
...
- `query` 会继续用结构化 JSON 返回行结果；MCP 文本内容里会带一条面向 user 的 SQL 回显，以及一条面向 assistant 的 JSON 镜像
