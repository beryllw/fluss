...
- `metrics.reporter.prometheus.filter-label-value-characters` - (optional) Specifies whether to filter label value characters, defaults to true. If enabled, all characters not matching `[a-zA-Z0-9:_]` will be replaced with an underscore, otherwise no characters will be replaced. Before disabling this option please ensure that your label values meet the [Prometheus requirements](https://prometheus.io/docs/concepts/data_model/#metric-names-and-labels).
